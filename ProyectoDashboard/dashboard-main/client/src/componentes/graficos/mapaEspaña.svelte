...
const nameMap = new Map(
  rawNameMap.map(([geo, csv]) => [normalize(geo), normalize(csv)])
);
  ...
  onMount(async () => {
    try {
      ...
      const datosCCAA = await res.json();

    
      const valoresPorCCAA = new Map(
        datosCCAA.map(d => {
            const normalizada = normalize(d.ccaa);
            return [normalizada, Number(d.centil_hijo)];
        })
        );
        console.log("Claves CSV normalizadas:", [...valoresPorCCAA.keys()]);
      ...
      const vals = datosCCAA
        .map(d => Number(d.centil_hijo))
        .filter(v => !isNaN(v));
      ...
      svg.selectAll("path")
        ...
        .attr("d", path)
        .attr("fill", d => {
          const geoNameRaw = d.properties.name;
          const geoKey = normalize(geoNameRaw);
          const csvKey = nameMap.get(geoKey) ?? geoKey; 
          const v = valoresPorCCAA.get(csvKey);

          console.log("GeoJSON:", geoNameRaw, "| key:", geoKey, "| CSV key:", csvKey, "| Valor:", v);
          return v !== undefined ? color(v) : "#ccc";
        })
        .attr("stroke", "#333")
